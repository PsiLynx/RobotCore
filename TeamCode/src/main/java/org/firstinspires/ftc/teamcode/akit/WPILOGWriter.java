...
import org.firstinspires.ftc.teamcode.akit.wpi.DataLogWriter;
...
public class WPILOGWriter implements LogDataReceiver {
  ...
  public WPILOGWriter(AdvantageScopeOpenBehavior openBehavior) {
    this(Globals.INSTANCE.isSimulation() ? defaultPathSim : defaultPathRio, openBehavior);
  }
  ...
  public WPILOGWriter() {
    this(
        Globals.INSTANCE.isSimulation() ? defaultPathSim : defaultPathRio,
  ...
  public void end() {
    ...
    switch (openBehavior) {
      case ALWAYS:
        shouldOpen = Globals.INSTANCE.isSimulation();
        break;
      case AUTO:
        shouldOpen = Globals.INSTANCE.isSimulation() && Logger.hasReplaySource();
        break;
      case NEVER:
      default:
        shouldOpen = false;
    ...
    if (autoRename) {
      ...
      if (logDate == null) {
        if ((table.get("DriverStation/DSAttached", false)
            ...
            || Globals.INSTANCE.isSimulation()) {
          if (dsAttachedTime == null) {
            dsAttachedTime = Globals.INSTANCE.getCurrentTime();
          } else if (Globals.INSTANCE.getCurrentTime() - dsAttachedTime
